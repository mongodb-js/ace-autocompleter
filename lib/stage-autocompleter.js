...
class StageAutoCompleter {
   ...
   * @param {String} env - The cluster environment.
   */
  constructor(version, textCompleter, fields, stageOperator, env) {
  ...
  getCompletions(editor, session, position, prefix, done) {
    ...
    if (this.stageOperator && this.stageOperator === MATCH) {
      ...
      return done(null, filter(this.version, expressions, prefix, this.env));
    }
  }
}
