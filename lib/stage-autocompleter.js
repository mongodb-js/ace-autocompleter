...
class StageAutoCompleter {
   ...
   * @param {boolean} isAtlas - Is the cluster in Atlas.
   * @param {boolean} isDataLake - Is the cluster Atlas Data Lake.
   */
  constructor(version, textCompleter, fields, stageOperator, isAtlas, isDataLake) {
  ...
  getCompletions(editor, session, position, prefix, done) {
    ...
    if (this.stageOperator && this.stageOperator === MATCH) {
      ...
      return done(null, filter(this.version, expressions, prefix, this.isAtlas, this.isDataLake));
    }
  }
}
