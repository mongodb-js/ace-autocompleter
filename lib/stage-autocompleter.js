...
class StageAutoCompleter {
  ...
  async getCompletions(editor, session, position, prefix, done) {
    ...
    const findCommnets = () => new Promise((resolve) => {
      value.replace(findCommentsRe, (match, mlc, slc, start) => {
        if (mlc && start < activePosition && activePosition <= (start + match.length)) {
          isComment = true;
        }
        if (slc && start < activePosition && activePosition <= (start + match.length)) {
          isComment = true;
        }
        resolve(match);
      });
